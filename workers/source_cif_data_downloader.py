...
TREE_CANOPY_FILENAME = 'tree_canopy'
BUILDING_FOOTPRINT_FILENAME = 'building_footprints'
DSM_FILENAME = 'alos_dsm'
DEM_FILENAME = 'nasa_dem'
RESAMPLED_DEM_FILENAME = 'nasa_dem_1m'
BUILDING_HEIGHT_FILENAME = 'building_height'
LAND_COVER_FILENAME = 'land_cover'
...
def get_lulc(tile_data_path, aoi_bounds):
    ...
    # Save data to file
    save_raster_file(aoi_LULC_to_solweig, tile_data_path, LAND_COVER_FILENAME)
...
def get_canopy_height(tile_data_path, aoi_bounds):
    ...
    save_raster_file(aoi_TreeCanopyHeight_float32, tile_data_path, TREE_CANOPY_FILENAME)
...
def get_dsm(tile_data_path, aoi_bounds):
    ...
    save_raster_file(aoi_AlosDSM, tile_data_path, DSM_FILENAME)
...
def get_dem(tile_data_path, aoi_bounds):
    ...
    save_raster_file(aoi_NasaDEM, tile_data_path, DEM_FILENAME)

    # resample to finer resolution of 1 meter
    dem_1m = resample_raster(aoi_NasaDEM, 1)

    save_raster_file(dem_1m, tile_data_path, RESAMPLED_DEM_FILENAME)
...
def get_building_footprints(tile_data_path, aoi_bounds):
    ...
    save_vector_file(aoi_OvertureBuildings, tile_data_path, BUILDING_FOOTPRINT_FILENAME)


def get_building_height(tile_data_path):
    aoi_OvertureBuildings = read_vector_file(tile_data_path, BUILDING_FOOTPRINT_FILENAME)
    aoi_AlosDSM = read_tiff_file(tile_data_path, DSM_FILENAME)
    aoi_NasaDEM = read_tiff_file(tile_data_path, DEM_FILENAME)
    aoi_NasaDEM_1m = read_tiff_file(tile_data_path, RESAMPLED_DEM_FILENAME)
    ...
    # Write to file
    save_vector_file(aoi_OvertureBuildings, tile_data_path, BUILDING_HEIGHT_FILENAME)
    ...
    # Save data to file
    save_raster_file(aoi_OvertureBuildings_raster, tile_data_path, BUILDING_HEIGHT_FILENAME)
