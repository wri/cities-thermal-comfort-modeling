...
from job_handler.graph_builder import _get_cif_features
from src.src_tools import _get_existing_tiles
    ...
    for index, config_row in processing_config_df.iterrows():
        ...
        if has_no_custom_features:
            ...
            if not isinstance(min_lon, float) or not isinstance(min_lat, float) or not isinstance(max_lon, float) or not isinstance(max_lat, float):
                msg = f'If there are no custom source tif files, then values in NewProcessingArea section must be defined in {CityData.filename_method_parameters_config}'
                invalids.append(msg)
